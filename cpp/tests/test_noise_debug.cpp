...
TEST_CASE("Debug NoiseOptimizer zero noise", "[debug-noise]") {
    ...
    std::cout << "Score: " << initial_score << std::endl;
    ...
    float final_total_violation = eval.intersection_violation;

    std::cout << "\n=== FINAL STATE (after 20 zero-noise iterations) ===" << std::endl;
    std::cout << "Score: " << final_score << std::endl;
    std::cout << "Total intersection_violation: " << final_total_violation << std::endl;
    std::cout << "Number of intersecting pairs: " << final_violations.size() << std::endl;
    ...
    for (size_t i = 0; i < sol.size(); ++i) {
        if (original_valid[i] != eval.solution.is_valid(i)) {
            params_changed = true;
            std::cout << "Tree " << i << " validity changed!" << std::endl;
            ...
            if (std::abs(p.pos.x - original_params[i].pos.x) > 1e-6f ||
                ...
                params_changed = true;
                std::cout << "Tree " << i << " params changed!" << std::endl;
            }
        }
    }
    std::cout << "Params changed: " << (params_changed ? "YES" : "NO") << std::endl;
    ...
    for (const auto& pair : all_pairs) {
        ...
        if (std::abs(orig_v - final_v) > 1e-6f || in_orig != in_final) {
            ...
            std::cout << "  Final violation: " << (in_final ? std::to_string(final_v) : "NOT PRESENT") << std::endl;
            ...
            std::cout << "  Ground truth sum: " << ground_truth << " (capped: " << std::min(ground_truth, 0.15f) << ")" << std::endl;
        }
    }

    std::cout << "\nTotal differing pairs: " << diff_count << std::endl;
    std::cout << "Score difference: " << (final_score - initial_score) << std::endl;
